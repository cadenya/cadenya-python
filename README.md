# Cadenya Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/cadenya.svg?label=pypi%20(stable))](https://pypi.org/project/cadenya/)

The Cadenya Python library provides convenient access to the Cadenya REST API from any Python 3.9+
...
> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install cadenya`
...
to add `CADENYA_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncCadenya` instead of `Cadenya` and use `await` with each API call:
...
import os
import asyncio
from cadenya import DefaultAioHttpClient
from cadenya import AsyncCadenya
...
for ai_provider_key in first_page.items:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `cadenya.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `cadenya.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `cadenya.APIError`.
...
except cadenya.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except cadenya.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except cadenya.APIStatusError as e:
...
You can enable logging by setting the environment variable `CADENYA_LOG` to `info`.

```shell
$ export CADENYA_LOG=info
```
...
```python
import httpx
from cadenya import Cadenya, DefaultHttpxClient

client = Cadenya(
    # Or use the `CADENYA_BASE_URL` env var
